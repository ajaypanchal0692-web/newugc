...
export default async function handler(request, response) {
  ...
  if (job.status === 'completed' || job.status === 'failed') return response.status(200).json({ ...job, storage: await store.status() });
  if (!job.providerJobId) return response.status(200).json({ ...job, storage: await store.status() });
  ...
  try {
    ...
    return response.status(200).json({ ...updated, storage: await store.status() });
  } catch (error) {
