...
export default async function handler(request, response) {
  ...
  try {
    ...
    const job = await service.submit({ projectId: body.projectId, prompt: body.prompt, durationSeconds: body.durationSeconds, aspectRatio: body.aspectRatio, referenceImage: body.referenceImage });
    return response.status(job.status === 'failed' ? 502 : 202).json(job);
  } catch (error) { return response.status(500).json({ error: error instanceof Error ? error.message : String(error) }); }
}
